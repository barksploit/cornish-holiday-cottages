...
class wpbenchmarkio {
	...
	var $object_cache_key_count = 500;
	var $object_cache_group_count = 25;
	...
	function calculate_finals($bench) {
		...
		if (isset($global_averages["thankyou"]) && isset($global_averages["my_score"])) {
			...
			$dsc .= "
				...
				<div class='wpio-flex-row'>
					<div class='wpio-flex-col col-read-more'><i>* Please check <u><a href='https://report.wpbenchmark.io/" . $bench["bench_code"] . "/' target=_blank>read more</a></u> page for details of page loading timings.</i></div>
				</div>
				":"")."
			";
	...
	function create_db_test_tables() {
		...
		$sql = "CREATE TABLE IF NOT EXISTS `".$this->dbtables["obj"]."` (
			  ...
			  `random_int` int(10) unsigned NOT NULL,
		...
		$sql = "ALTER TABLE `".$this->dbtables["obj"]."` MODIFY `o_id` int(10) unsigned NOT NULL AUTO_INCREMENT;";
		$wpdb->query($sql);
		...
		$sql = "ALTER TABLE `".$this->dbtables["prop"]."` MODIFY `p_id` bigint(20) unsigned NOT NULL AUTO_INCREMENT;";
		$wpdb->query($sql);
		...
		$sql="ALTER TABLE `".$this->dbtables["log"]."` MODIFY `l_id` bigint(20) unsigned NOT NULL AUTO_INCREMENT;";
		$wpdb->query($sql);
	...
	function test_db_simple() {
		...
		for ($i=0;$i<400;$i++) {
			...
			$random_data = $wpdb->get_results("select * from ".$this->dbtables["obj"]." where random_int=".rand(1,1000)." order by RAND();", ARRAY_A );
			foreach($random_data as $r) {
				$o_properties = $wpdb->get_results("select * from ".$this->dbtables["prop"]." where o_id=".$r["o_id"].";", ARRAY_A );						
			}
			foreach($random_data as $r) {
				$o_properties = $wpdb->get_results("select * from ".$this->dbtables["prop"]." where o_id=".$r["o_id"].";", ARRAY_A );						
			}
		...
		$full_data = $wpdb->get_results("select * from ".$this->dbtables["obj"]." order by RAND()", ARRAY_A );
		foreach($full_data as $r) {
			...
			$tmp_properties = $wpdb->get_results("select * from ".$this->dbtables["prop"]." where o_id=".$r["o_id"].";", ARRAY_A );	
		...
		for ($i=0;$i<50;$i++) {
			...
			for ($j=0;$j<50;$j++) {
				...
				foreach($sql_result as $r) {
					$sql_result_2 = $wpdb->get_results("select * from ".$this->dbtables["obj"]." where o_id=".$r["o_id"], ARRAY_A);
				}
			}
	...
	function get_use_data() {
		...
		Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed faucibus tempor ex et tincidunt. Duis mollis risus vel congue viverra. Mauris interdum porta ex, et rutrum nulla euismod tincidunt. Ut tincidunt lorem eu libero vehicula, nec bibendum risus condimentum. Vestibulum mi felis, ornare vitae velit sit amet, dictum varius purus. Nam blandit suscipit semper. Nam sem arcu, aliquam quis tellus in, gravida ullamcorper eros.";
		$use_data[1] = "Nullam vel odio pretium, sodales diam at, malesuada magna. Fusce nec malesuada elit. Cras ultricies ipsum sed mollis tristique. Nulla egestas eu ligula vitae faucibus. Nulla facilisi. Pellentesque convallis nunc volutpat arcu pretium, et sollicitudin augue tincidunt. In posuere urna orci, et sodales metus porttitor sed. Curabitur lacinia bibendum diam nec cursus. Suspendisse sollicitudin ipsum diam, ac varius risus dictum at. Fusce semper urna vel magna lacinia condimentum. Nulla eget consectetur nulla. Nunc a turpis sollicitudin, suscipit nisi eget, bibendum massa. Donec et sem magna. Donec ultrices dignissim velit. Etiam eros massa, molestie nec felis et, tristique tincidunt mauris.
...
Quisque dapibus odio et faucibus vestibulum. Vivamus sed pretium neque. Aenean libero ipsum, bibendum sed dui non, faucibus ullamcorper velit. Integer rhoncus finibus massa eget hendrerit. In hac habitasse platea dictumst. Nam nec gravida felis, eget mollis dui. Donec odio lorem, luctus vel nunc in, fringilla porta tortor. Morbi blandit risus a nisl iaculis ultrices. Aliquam purus augue, lobortis ac nibh id, pulvinar imperdiet augue. Fusce vehicula tellus purus, non interdum purus pellentesque ut. Pellentesque ac massa ultrices, aliquet massa sit amet, hendrerit eros. Vestibulum mollis, elit ut pulvinar pulvinar, ante ex pulvinar nibh, vitae vehicula dolor dui a magna. Curabitur sit amet purus vitae est dignissim mattis. Cras sed rutrum sapien. Integer posuere lobortis tellus. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. 
...
Quisque dapibus odio et faucibus vestibulum. Vivamus sed pretium neque. Aenean libero ipsum, bibendum sed dui non, faucibus ullamcorper velit. Integer rhoncus finibus massa eget hendrerit. In hac habitasse platea dictumst. Nam nec gravida felis, eget mollis dui. Donec odio lorem, luctus vel nunc in, fringilla porta tortor. Morbi blandit risus a nisl iaculis ultrices. Aliquam purus augue, lobortis ac nibh id, pulvinar imperdiet augue. Fusce vehicula tellus purus, non interdum purus pellentesque ut. Pellentesque ac massa ultrices, aliquet massa sit amet, hendrerit eros. Vestibulum mollis, elit ut pulvinar pulvinar, ante ex pulvinar nibh, vitae vehicula dolor dui a magna. Curabitur sit amet purus vitae est dignissim mattis. Cras sed rutrum sapien. Integer posuere lobortis tellus. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. ";
	...
	function fill_object_cache() {
		...
		for ($j=0;$j<$this->object_cache_group_count;$j++) {
			...
			for ($i=0;$i<$this->object_cache_key_count;$i++) {
				$this_data = $use_data[(($i+$j)%3)];

				wp_cache_add("temp_".$i, $this_data, "wpbenchmark-".$j);

				# fill up big data array's random element 
				$big_array[rand(0,99)][rand(0,9)] = $this_data;
			}

			wp_cache_add("temp_big", $big_array, "wpbenchmark-".$j);
		} # end of group J
	...
	function test_oc_persistent_write() {
		...
		$this->local_wp_cache_flush();	# empty cache
		$this->local_add_test_variable(); # add very important variable, that we are checking
	...
	function test_oc_persistent_mixed() {
		...
		for($i=0;$i<400000;$i++) {
			if ($i%50==0) {
				...
				$test_value = $use_data[$i%3];
				wp_cache_set("temp_".$random_key, $test_value, "wpbenchmark-".rand(0,$max_object_cache_group));
			} else if ($i%40==0) {
				if (rand(0,1)==0)
					$value = wp_cache_get("temp_big", "wpbenchmark-".rand(0,$max_object_cache_group));
				else
