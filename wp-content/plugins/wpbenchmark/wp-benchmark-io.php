...
class wp_benchmark_io {
	...
	public static function admin_menu_options() {
		...
		if (current_user_can("install_plugins")) {
			...
			if (isset($_POST["doa"]) && $_POST["doa"]=="save_settings") {
...
if ( is_admin() ) {
	...
	if (!isset($_POST["start_benchmark"]))
		$_POST["start_benchmark"]=0;
	if (!isset($_POST["run_next_step"]))
		$_POST["run_next_step"]=0;
	if (!isset($_POST["get_finals"]))
		$_POST["get_finals"]=0;
	if (!isset($_POST["skip_failed_test"]))
		$_POST["skip_failed_test"]=0;
	if (!isset($_POST["clear_local_results"]))
		$_POST["clear_local_results"]=0;


	if ($_POST["start_benchmark"]==1 || $_POST["run_next_step"]==1 || $_POST["get_finals"]==1 || $_POST["skip_failed_test"]==1 || $_POST["clear_local_results"]==1) {




		if ($_POST["clear_local_results"]==1) {
		...
		if ($settings["accept_terms"]==1 && $settings["gdpr_consent"]==1) {
			try {
				...
				} else if ($_POST["run_next_step"]==1 || $_POST["skip_failed_test"]==1) {
	...
	#add_action( 'wp_enqueue_scripts', array('wp_benchmark_io', 'custom_add_google_fonts') );
}
