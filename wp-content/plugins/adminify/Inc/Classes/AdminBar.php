...
class AdminBar extends AdminSettingsModel
{
    ...
    public function jltwp_adminify_admin_bar_css()
    {
        ...
        if ( !empty($new_btn_colors['bg_color']) ) {
            $output_css .= '.wp-adminify #wpadminbar #wp-admin-bar-root-default #wp-admin-bar-new-content > .ab-item, .wp-adminify #wpadminbar #wp-admin-bar-root-default #wp-admin-bar-new-content:hover > .ab-item { background-color:' . $new_btn_colors['bg_color'] . '; border: 1px solid ' . $new_btn_colors['bg_color'] . ';}';
        }
