...
class ActivityLogs
{
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            ...
            $wpdb->adminify_activity_logs = $wpdb->prefix . 'adminify_activity_logs';
        }
    ...
    {
        if (empty($_GET['page']) || $_GET['page'] !== 'adminify-activity-logs') return;
        if (empty($_GET['action']) || $_GET['action'] == -1) return;
        if (empty($_GET['log_id'])) return;
    ...
    public function jltwp_adminify_get_list_table()
    {
        if (is_null($this->_list_table)) {
            $this->_list_table = new Adminify_Activity_Log_List_Table(array('screen' => $this->_screens['adminify_log']));
