...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  data: function data() {
    ...
    return _ref = {
      ...
      filter_folder: '',
      total_uncat_posts: ''
    }, _defineProperty(_ref, "total_uncat_posts", ''), _defineProperty(_ref, "folder_select_toggle", false), _defineProperty(_ref, "create_folder_popup", false), _defineProperty(_ref, "new_folder_name", null), _defineProperty(_ref, "create_folder_error", null), _defineProperty(_ref, "rename_folder_popup", false), _defineProperty(_ref, "rename_folder_name", null), _defineProperty(_ref, "rename_folder_error", null), _defineProperty(_ref, "delete_folder_popup", false), _defineProperty(_ref, "delete_folder_error", null), _defineProperty(_ref, "color_tags", ['#7B61FF', '#0347FF', '#F24AE1', '#ED2E7E', '#FFC804', '#00BA88']), _defineProperty(_ref, "active_color_tag", '#7B61FF'), _defineProperty(_ref, "currentView", 'all'), _defineProperty(_ref, "sort", 'a-z'), _defineProperty(_ref, "is_pro", !!wp_adminify__folder_data.is_pro), _defineProperty(_ref, "pro_notice", wp_adminify__folder_data.pro_notice), _defineProperty(_ref, "total_posts", {
      'auto-draft': 0,
      'draft': 0,
      'future': 0,
      'inherit': 0,
  ...
  methods: {
    ...
    get_folder_url: function get_folder_url(folder) {
      if (folder == 'all') {
        if (this.post_type == 'attachment') return 'upload.php';
        if (this.post_type == 'post') return this.adminurl + 'edit.php';
        return this.adminurl + "edit.php?post_type=".concat(this.post_type);
      }

      var folderSlug = folder == 'uncategorized' ? '-1' : folder.slug;
      if (this.post_type == 'attachment') return this.adminurl + "upload.php?".concat(this.post_type_tax, "=").concat(folderSlug);
      if (this.post_type == 'post') return this.adminurl + "edit.php?".concat(this.post_type_tax, "=").concat(folderSlug);
      return this.adminurl + "edit.php?post_type=".concat(this.post_type, "&").concat(this.post_type_tax, "=").concat(folderSlug);
    },
    ...
    init_draggable_events: function init_draggable_events() {
      ...
      $('.attachments-browser li.attachment:not(.ui-draggable)').draggable({
        ...
        helper: function helper(event, ui) {
          ...
          selectedItems = selectedItems < 2 ? '1 Item' : "".concat(selectedItems, " Items");
          return $("<div class='adminify--selected-items'><span class='total-post-count'>".concat(selectedItems, " Selected</span></div>"));
        }
      });
    },
    init_droppable_events: function init_droppable_events() {
      ...
      $('.folder--lists li a:not(.ui-droppable), .folder--stats li.folder--single-uncategorized a:not(.ui-droppable)').droppable({
        ...
        drop: function drop(event, ui) {
          var folderID = $(this).parent("li").data('folder');
          if (!folderID) return;
          var postIDs = [];
          ...
          if (!ui.draggable.hasClass('attachment')) {
            var mode = $('input[name=post_view]').first().val() || 'list';
            $.ajax({
              url: wp_adminify__folder_data.ajaxurl,
              method: 'post',
              data: {
    ...
    refresh_folders_data: function refresh_folders_data() {
      ...
      setTimeout(function () {
        _this6.init_droppable_events();

        _this6.init_draggable_events();
      }, 200);
    },
  ...
  computed: {
    ...
    filtered_folders: function filtered_folders() {
      ...
      if (this._filter_folder) {
        var _folders = folders.filter(function (_folder) {
          return _folder.name.toLowerCase().search(_this7._filter_folder.toLowerCase()) > -1;
        });
    ...
    total_posts_count: function total_posts_count() {
      ...
      this.get_allowed_status().forEach(function (status) {
        total += Number(_this8.total_posts[status]);
      });
      return total;
    },
  ...
  watch: {
    filter_folder: function filter_folder() {
      ...
      setTimeout(function () {
        _this9.init_draggable_events();

        _this9.init_droppable_events();
      }, 50);
    }
  }
});
